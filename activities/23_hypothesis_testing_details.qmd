...
> Conclusions: The `RiverWacamaw` coefficient represents a difference in where the Lumber and Wacamaw River lines "start", and so the conclusions we draw from looking at this coefficient should be taken alongside our inspection of the interaction coefficient in Part e. There seems to be a modest but real interaction between River and Length, so `River` should be included in the model for that reason.
...
> 
> To consider the question of river as a confounder more directly, let's use plots. When looking at the relationship between `River` and `Length`, we don't see much of a difference between rivers in fish length. So there doesn't seem to be a connection between river and length on a causal diagram. `River` + `Concen`. When looking at the relationship between `River` and `Concen`, there is slightly higher concentration in the Wacamaw River.
>
> So `River` is acting more as a precision variable than a confounder and is still important to include in the model in an interaction term with `Length`.

```{r}
ggplot(fish, aes(x = River, y = Length)) + 
    geom_boxplot() +
    theme_classic()

ggplot(fish, aes(x = River, y = Concen)) + 
    geom_boxplot() +
    theme_classic()
```
